...
export function generateFixedIncomeData(numPositions = 10000) {
  ...
  function getRandomNumber(min: number, max: number, decimals = 2) {
  ...
  function getRandomIssuer(securityType: string) {
  ...
  function generateISIN(countryCode: string) {
    const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
  ...
  function calculateRiskMetrics(securityInfo: any) {
  ...
  for (let i = 0; i < numPositions; i++) {
    ...
    // Determine sector
    let sector;
    if ('sector' in issuer && issuer.sector) {
      sector = issuer.sector;
    } else if (sectors[securityType as keyof typeof sectors]) {
      sector = getRandomElement(sectors[securityType as keyof typeof sectors]);
    } else {
      sector = "Other";
    }
    ...
    const position: any = {
      ...
      collateralType: securityType === "MBS" || securityType === "ABS" || securityType === "CMBS" ? 
                      (securityType === "MBS" ? "Residential Mortgages" : 
                       securityType === "ABS" ? getRandomElement(sectors["ABS" as keyof typeof sectors]) : 
                       getRandomElement(sectors["CMBS" as keyof typeof sectors])) : "",
    ...
    Object.assign(position, {
      ...
      CDS5Y: securityType === "Corporate Bond" || securityType === "Sovereign" ? baseSpread * 100 + getRandomNumber(-20, 20, 0) : null,
